...
def PDFInsightAgent(pdf_file, use_vision: bool = False) -> str:
    ...
    try:
        if use_vision:
            # Use Claude's native PDF processing with vision
            pdf_base64 = convert_pdf_to_base64(pdf_file)
            
            response = client.messages.create(
                model=current_config.MODEL_NAME,
                max_tokens=current_config.INSIGHTS_MAX_TOKENS * 2,  # PDFs may need more tokens
                temperature=current_config.INSIGHTS_TEMPERATURE,
                messages=[{
                    "role": "user",
                    "content": [
...
def main():
    ...
    if "file_type" not in st.session_state:
        st.session_state.file_type = "csv"  # Track current file type
    ...
    with left:
        ...
        if selected_model != st.session_state.current_model:
            ...
            # Regenerate insights based on file type
            if file is not None:
        ...
        if file:
            ...
            if st.session_state.file_type == "csv":
                if ("df" not in st.session_state) or file_changed:
                    ...
                    # Clear PDF data if switching from PDF
                    if "pdf_file" in st.session_state:
                        del st.session_state.pdf_file
                ...
                if ("pdf_file" not in st.session_state) or file_changed:
                    ...
                    # Clear CSV data if switching from CSV
                    if "df" in st.session_state:
                        del st.session_state.df
        ...
        if "df" in st.session_state:
            ...
            if user_q := st.chat_input("Ask about the PDF document…"):
                ...
                with st.spinner("Analyzing…"):
                    ...
                    conversation = []
                    for msg in st.session_state.pdf_messages[:-1]:  # Exclude the current question
                        conversation.append({"role": msg["role"], "content": msg["content"]})
                    
                    # Add the PDF and current question
                    pdf_use_vision = st.session_state.get("pdf_use_vision", True)
                    
                    if pdf_use_vision:
                        # Use Claude's native PDF processing
                        pdf_base64 = convert_pdf_to_base64(st.session_state.pdf_file)
                        
                        # Build the message content with PDF and question
                        message_content = [
                            {
                                "type": "document",
                                "source": {
                        ...
                        # If there's conversation history, we need to handle it differently
                        if conversation:
